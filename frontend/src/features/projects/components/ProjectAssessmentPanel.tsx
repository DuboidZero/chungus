  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsFormOpen(!isFormOpen)} className="text-sm font-medium text-emerald-600 hover:text-emerald-700">
      ...
      {isFormOpen && (
        <Card className="border-emerald-200">
          <CardContent className="p-4 sm:p-5 bg-emerald-50/30">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-on-surface-variant mb-1">Assessment Title</label>
                  <input required type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full p-2 text-sm bg-white border border-outline-variant rounded-md focus:ring-2 focus:ring-emerald-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-on-surface-variant mb-1">Score</label>
                  <input required type="number" min="0" value={form.score} onChange={e => setForm({...form, score: e.target.value})} className="w-full p-2 text-sm bg-white border border-outline-variant rounded-md focus:ring-2 focus:ring-emerald-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-on-surface-variant mb-1">Max Score</label>
                  <input required type="number" min="1" value={form.maxScore} onChange={e => setForm({...form, maxScore: e.target.value})} className="w-full p-2 text-sm bg-white border border-outline-variant rounded-md focus:ring-2 focus:ring-emerald-500" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1">Comments</label>
                <textarea required value={form.comments} onChange={e => setForm({...form, comments: e.target.value})} className="w-full h-20 p-2 text-sm bg-white border border-outline-variant rounded-md focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
      ...
      {marks.length === 0 ? (
        <div className="text-center py-6 border border-dashed border-outline-variant rounded-lg">
          <p className="text-sm text-on-surface-variant font-medium">No assessments logged yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {marks.map(mark => (
            <Card key={mark.id} className="border-outline-variant">
              <CardContent className="p-4 flex gap-4 justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-on-surface-variant mb-2">Evaluated by {mark.teacherName} on {new Date(mark.date).toLocaleDateString()}</p>

...
export function SearchFilterBar({ onFilterChange, onSearch, initialState }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-surface-container-low rounded-lg border border-slate-200 dark:border-outline-variant p-3 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-surface-container-low border border-slate-200 dark:border-outline-variant rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary text-slate-900 dark:text-slate-100"
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors border ${
            hasActiveFilters || isExpanded 
              ? 'bg-surface-container-low dark:bg-surface-container-low text-primary dark:text-primary border-outline-variant dark:border-outline-variant'
              : 'bg-white dark:bg-transparent text-slate-600 dark:text-slate-400 border-slate-200 dark:border-outline-variant hover:bg-slate-50 dark:hover:bg-surface-container'
          }`}
        >
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-outline-variant grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full p-2 text-sm bg-slate-50 dark:bg-surface-container-low border border-slate-200 dark:border-outline-variant rounded text-slate-900 dark:text-slate-100 focus:outline-none focus:border-primary"
            >
